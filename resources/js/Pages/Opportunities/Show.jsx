...
export default function ShowOpportunity({ opportunity }) {
                    ...
                    <div className="space-y-6 lg:col-span-2">
                        ...
                        <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                            <div className="mb-4 flex items-start justify-between">
                                ...
                                <div className="flex gap-2">
                                    <span className={`inline-flex rounded-full px-3 py-1 text-sm font-semibold ${getStatusColor(opportunity.status)}`}>
                                        {opportunity.status.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                                    </span>
                                    <span className={`inline-flex rounded-full px-3 py-1 text-sm font-semibold ${getStageColor(opportunity.stage)}`}>
                                        {opportunity.stage.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(opportunity.value)}</p>
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Probability</p>
                                    <div className="mt-1 flex items-center">
                                        <span className="text-2xl font-bold text-gray-900 dark:text-white">{opportunity.probability}%</span>
                        ...
                        {opportunity.lead && (
                            <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                                <div className="mb-4 flex items-center justify-between">
                                    ...
                                    <Link
                                        href={route('leads.show', opportunity.lead.id)}
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center">
                                        ...
                                        <span className="text-gray-900 dark:text-white">{opportunity.lead.full_name}</span>
                                    </div>
                                    {opportunity.lead.email && (
                                        <div className="flex items-center">
                                            ...
                                            <a href={`mailto:${opportunity.lead.email}`} className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                                                {opportunity.lead.email}
                                            </a>
                                        </div>
                                    )}
                                    {opportunity.lead.phone && (
                                        <div className="flex items-center">
                                            ...
                                            <a href={`tel:${opportunity.lead.phone}`} className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                                                {opportunity.lead.phone}
                                            </a>
                                        </div>
                                    )}
                        ...
                        {opportunity.property && (
                            <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                                <div className="mb-4 flex items-center justify-between">
                                    ...
                                    <Link
                                        href={route('properties.show', opportunity.property.id)}
                                        className="text-sm text-indigo-600 hover:text-indigo-700 dark:text-indigo-400"
                                    >
                                        View Property →
                                    </Link>
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <p className="mt-1 text-gray-900 dark:text-white">{opportunity.property.title}</p>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Type</p>
                                            <p className="mt-1 text-gray-900 dark:text-white">
                                                {opportunity.property.type.charAt(0).toUpperCase() + opportunity.property.type.slice(1)}
                                            </p>
                                        </div>
                                        <div>
                                            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Price</p>
                                            <p className="mt-1 text-gray-900 dark:text-white">{formatCurrency(opportunity.property.price)}</p>
                                        </div>
                                    </div>
                                    {opportunity.property.city && (
                                        <div>
                                            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Location</p>
                                            <p className="mt-1 text-gray-900 dark:text-white">{opportunity.property.city}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-gray-900 dark:text-white">{formatDate(opportunity.expected_close_date)}</p>
                                </div>
                                {opportunity.actual_close_date && (
                                    <div>
                                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Actual Close Date</p>
                                        <p className="mt-1 text-gray-900 dark:text-white">{formatDate(opportunity.actual_close_date)}</p>
                                    </div>
                                )}
                                {opportunity.assigned_to_user && (
                                    <div>
                                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Assigned To</p>
                                        <p className="mt-1 text-gray-900 dark:text-white">{opportunity.assigned_to_user.name}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {opportunity.notes && (
                            <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                                ...
                                <p className="text-gray-900 dark:text-white whitespace-pre-wrap">{opportunity.notes}</p>
                            </div>
                        )}
                        ...
                        {opportunity.status === 'lost' && opportunity.lost_reason && (
                            <div className="rounded-lg bg-red-50 p-6 shadow dark:bg-red-900/20">
                                ...
                                <p className="text-red-800 dark:text-red-300 whitespace-pre-wrap">{opportunity.lost_reason}</p>
                            </div>
                        )}

                        {/* Activity Log */}
                        {opportunity.audits && opportunity.audits.length > 0 && (
                            <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                                ...
                                <div className="flow-root">
                                    <div className="-mb-8 max-h-96 overflow-y-auto">
                                        {opportunity.audits.map((audit, index) => (
                                            <div key={audit.id} className="relative pb-8">
                                                {index !== opportunity.audits.length - 1 && (
                                                    <span
                            ...
                            <div className="space-y-4">
                                {opportunity.created_by_user && (
                                    <div>
                                        ...
                                        <p className="mt-1 text-gray-900 dark:text-white">{opportunity.created_by_user.name}</p>
                                    </div>
                                )}
                                <div>
                                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Created At</p>
                                    <p className="mt-1 text-gray-900 dark:text-white">{formatDate(opportunity.created_at)}</p>
                                </div>
                                {opportunity.updated_at !== opportunity.created_at && (
                                    <div>
                                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Last Updated</p>
                                        <p className="mt-1 text-gray-900 dark:text-white">{formatDate(opportunity.updated_at)}</p>
                                    </div>
                                )}
                            </div>
                        </div>
