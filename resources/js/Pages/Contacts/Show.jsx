...
export default function Show({ contact }) {
    ...
    return (
        <AuthenticatedLayout header={contact.full_name}>
            <Head title={`Lead - ${contact.full_name}`} />
                            ...
                            <div className="p-6">
                                <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.full_name}</dd>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.job_title || 'N/A'}</dd>
                                    </div>
                                </dl>
                            </div>
                        </div>
                            ...
                            <div className="p-6">
                                <dl className="grid grid-cols-1 gap-6">
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.company || 'N/A'}</dd>
                                    </div>
                                </dl>
                            </div>
                        </div>
                                    ...
                                    <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                        {contact.address && (
                                            <div className="sm:col-span-2">
                                                ...
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.address}</dd>
                                            </div>
                                        )}
                                        {contact.city && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">City</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.city}</dd>
                                            </div>
                                        )}
                                        {contact.state && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">State</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.state}</dd>
                                            </div>
                                        )}
                                        {contact.postal_code && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Postal Code</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.postal_code}</dd>
                                            </div>
                                        )}
                                        {contact.country && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Country</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.country}</dd>
                                            </div>
                                        )}
                                    </dl>
                                </div>
                        ...
                        <div className="overflow-hidden rounded-lg bg-white shadow dark:bg-gray-800">
                            ...
                            <div className="p-6 space-y-4">
                                <div>
                                    ...
                                    <dd className="mt-1">
                                        <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${getStatusColor(contact.status)}`}>
                                ...
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                                        {contact.source.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                                    </dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Estimated Value</dt>
                                    <dd className="mt-1 text-sm font-semibold text-gray-900 dark:text-white">
                                        {contact.estimated_value ? `$${parseFloat(contact.estimated_value).toLocaleString()}` : 'N/A'}
                                    </dd>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                                        {contact.last_contact_date ? formatDate(contact.last_contact_date) : 'N/A'}
                                    </dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Created By</dt>
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">{contact.created_by?.name}</dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Created At</dt>
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">{formatDate(contact.created_at)}</dd>
                                </div>
                            </div>
                        </div>
