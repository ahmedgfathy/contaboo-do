...
export default function Show({ activity }) {
    ...
    return (
        <AuthenticatedLayout header={activity.full_name}>
            <Head title={`Lead - ${activity.full_name}`} />
                    ...
                    <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="p-6">
                                <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.full_name}</dd>
                                    </div>
                                    <div>
                                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Email</dt>
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                                            <a href={`mailto:${activity.email}`} className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                                                {activity.email}
                                            </a>
                                        </dd>
                                    </div>
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                                            {activity.phone ? (
                                                <a href={`tel:${activity.phone}`} className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                                                    {activity.phone}
                                                </a>
                                            ) : (
                                                'N/A'
                                    ...
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.job_title || 'N/A'}</dd>
                                    </div>
                                </dl>
                            </div>
                        </div>
                            ...
                            <div className="p-6">
                                <dl className="grid grid-cols-1 gap-6">
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.company || 'N/A'}</dd>
                                    </div>
                                </dl>
                            </div>
                        </div>
                                    ...
                                    <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                        {activity.address && (
                                            <div className="sm:col-span-2">
                                                ...
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.address}</dd>
                                            </div>
                                        )}
                                        {activity.city && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">City</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.city}</dd>
                                            </div>
                                        )}
                                        {activity.state && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">State</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.state}</dd>
                                            </div>
                                        )}
                                        {activity.postal_code && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Postal Code</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.postal_code}</dd>
                                            </div>
                                        )}
                                        {activity.country && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Country</dt>
                                                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.country}</dd>
                                            </div>
                                        )}
                                    </dl>
                                </div>
                        ...
                        {activity.notes && (
                            <div className="overflow-hidden rounded-lg bg-white shadow dark:bg-gray-800">
                                ...
                                <div className="p-6">
                                    <p className="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{activity.notes}</p>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="overflow-hidden rounded-lg bg-white shadow dark:bg-gray-800">
                            ...
                            <div className="p-6 space-y-4">
                                <div>
                                    ...
                                    <dd className="mt-1">
                                        <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${getStatusColor(activity.status)}`}>
                                ...
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                                        {activity.source.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                                    </dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Estimated Value</dt>
                                    <dd className="mt-1 text-sm font-semibold text-gray-900 dark:text-white">
                                        {activity.estimated_value ? `$${parseFloat(activity.estimated_value).toLocaleString()}` : 'N/A'}
                                    </dd>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                                        {activity.last_contact_date ? formatDate(activity.last_contact_date) : 'N/A'}
                                    </dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Created By</dt>
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">{activity.created_by?.name}</dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Created At</dt>
                                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">{formatDate(activity.created_at)}</dd>
                                </div>
                            </div>
                        </div>
